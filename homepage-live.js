(function(){
  ...
  function renderJobs(){
    ...
    if(!filtered.length){el.innerHTML='<div class="ax-empty"><strong>No active jobs in this category.</strong>View all Current Jobs for the full list.</div>';return}
  ...
  function renderNews(){
    ...
    el.innerHTML=filtered.slice(0,5).map(n=>{
      ...
      return `<article class="ax-news-item"><div class="ax-news-img">${img?`<img src="${esc(img)}" alt="" loading="lazy" onerror="this.remove()">`:''}</div><div><div class="ax-news-meta"><span class="ax-news-cat">${cat}</span><span>${esc(n.published_date||'')}</span></div><h3>${esc(n.title||'Current affairs update')}</h3><p>${esc(String(n.summary||'').slice(0,105))}${String(n.summary||'').length>105?'…':''}</p></div></article>`;
  ...
  function renderBooks(data){
    ...
    el.innerHTML=data.slice(0,4).map(b=>{
      ...
      return `<article class="ax-book-mini"><div class="ax-book-cover">${cover?`<img src="${esc(cover)}" alt="${title}" loading="lazy" onerror="this.remove()">`:''}<div class="ax-book-fallback" ${cover?'style="display:none"':''}><small>${esc(b.category||'Competitive exams')}</small><strong>${title}</strong></div></div><h3>${title}</h3><div class="ax-book-buttons">${amazon?`<a class="ax-amz" href="${esc(amazon)}" target="_blank" rel="sponsored noopener">Amazon</a>`:''}${flip?`<a class="ax-flip" href="${esc(flip)}" target="_blank" rel="sponsored noopener">Flipkart</a>`:''}</div></article>`;
    }).join('');
  }

  function renderFocus(){
    const el=$('focusQuestion');if(!el)return; const q=state.focus;
    if(!q){el.innerHTML='<div class="ax-empty"><strong>No published practice questions are available.</strong>Open Practice to continue.</div>';return}
    ...
    $('focusSubmit').addEventListener('click',()=>{
      const r=$('focusResult'); if(state.focusSelected===null||state.focusSelected===undefined){r.className='ax-focus-result bad';r.textContent='Choose an answer first.';return}
      const correct=String(q.answer||'A').toUpperCase().charCodeAt(0)-65;
      if(state.focusSelected===correct){r.className='ax-focus-result';r.textContent='Correct. '+String(q.explanation||'Keep going.')}else{r.className='ax-focus-result bad';r.textContent='Not quite. Correct answer: '+String(q.answer||'A')+'. '+String(q.explanation||'Review this topic in Practice.')} }
    )
  }

  async function loadCounts(){
    try{const r=await db.from('questions').select('*',{count:'exact',head:true}).eq('status','published');const n=r.count||10000;const txt=n>=1000?(Math.floor(n/1000)+'k+'):String(n);if($('homeQuestionCount'))$('homeQuestionCount').textContent=txt;if($('statQuestions'))$('statQuestions').textContent=txt}catch(_){}}
  ...
  async function loadNews(){
    ...
    if(!error){state.news=data||[];renderNews()}else{$('homeNewsList').innerHTML='<div class="ax-empty"><strong>Current Affairs are temporarily unavailable.</strong><a href="current-affairs.html">Open Current Affairs →</a></div>'}
  }
  async function loadBooks(){
    const {data,error}=await db.from('books_v1').select('id,title,author,description,exam_name,category,cover_url,amazon_url,flipkart_url,featured,created_at').eq('status','published').order('featured',{ascending:false}).order('created_at',{ascending:false}).limit(8);
    if(!error)renderBooks(data||[]);else $('homeBooksShelf').innerHTML='<div class="ax-empty"><strong>Books are temporarily unavailable.</strong><a href="books.html">Open Books →</a></div>';
  }
  async function loadFocus(){
    const {data,error}=await db.from('questions').select('id,question,option_a,option_b,option_c,option_d,answer,explanation,subject_id,subjects(name)').eq('status','published').limit(40);
    if(!error&&data?.length){state.focus=data[Math.floor(Math.random()*data.length)];renderFocus()}
    else renderFocus();
  }
  function initFilters(){
    document.querySelectorAll('#jobFilters button').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#jobFilters button').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.jobCat=b.dataset.cat;renderJobs()}));
    document.querySelectorAll('#newsFilters button').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('#newsFilters button').forEach(x=>x.classList.remove('active'));b.classList.add('active');state.newsCat=b.dataset.cat;renderNews()}));
  }
  ...
  function initStreak(){
    const key='axomprep_visit_dates_v1';const today=new Date();const id=today.toISOString().slice(0,10);let arr=[];try{arr=JSON.parse(localStorage.getItem(key)||'[]')}catch(_){arr=[]}if(!arr.includes(id))arr.push(id);arr=arr.slice(-30);try{localStorage.setItem(key,JSON.stringify(arr))}catch(_){}
    let streak=0;const d=new Date(today);while(true){const s=d.toISOString().slice(0,10);if(arr.includes(s)){streak++;d.setDate(d.getDate()-1)}else break}if($('streakDays'))$('streakDays').textContent=streak;
    const days=['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];const start=new Date(today);const dow=(start.getDay()+6)%7;start.setDate(start.getDate()-dow);let html='';for(let i=0;i<7;i++){const x=new Date(start);x.setDate(start.getDate()+i);const s=x.toISOString().slice(0,10);html+=`<span><i class="${arr.includes(s)?'done':''}">${arr.includes(s)?'✓':''}</i>${days[i]}</span>`}if($('streakWeek'))$('streakWeek').innerHTML=html;
  }
  document.addEventListener('DOMContentLoaded',()=>{initMenu();initFilters();initStreak();loadCounts();loadJobs();loadNews();loadBooks();loadFocus()});
})();
